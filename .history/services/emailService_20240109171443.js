...
let getBodyHTMLEmail = (dataSend) => {
    ...
    if (dataSend.language === 'vi') {
        ...
        <p>Bạn nhận được thư này vì đã đặt lịch khám bệnh online trên web khamchuabenh</p>
...
let getBodyHTMLEmailDoneRemedy = (dataSend) => {
    let result = ''
    if (dataSend.language === 'vi') {
        result =
            `
        <h3> Xin chào ${dataSend.patientName} !</h3 >
        <p>Bạn nhận được thư này vì đã đặt lịch khám bệnh online trên web khamchuabenh</p>
     <p>Xin cảm ơn quý khách đã sử dụng dịch vụ của chúng tôi. Chúc quý khách mạnh khỏe!</p>
    ...
    if (dataSend.language === 'en') {
        result =
            `
       <h3> Hello ${dataSend.PatientName} !</h3 >
        <p>You are receiving this letter because you made an online medical appointment on khamchuabenh website</p>
     <p>Thank you for using our service. Wishing you good health!</p>
...
let sendDoneAttachment = async (dataSend) => {
    return new Promise(async (resolve, reject) => {
        try {
            let transporter = nodemailer.createTransport({
                host: "smtp.gmail.com",
            ...
            let info = await transporter.sendMail({
                ...
                to: dataSend.email,
                subject: "Dear",
                html: getBodyHTMLEmailDoneRemedy(dataSend),
                attachments: [
                    {
                        filename: `remedy-${dataSend.patientId}-${new Date().getTime()}.png`,
                        content: dataSend.imgBase64.split("base64,")[1],
...
export default {
    ...
    sendDoneAttachment: sendDoneAttachment,
